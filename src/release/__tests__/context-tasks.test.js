...
describe('TaskRunner', () => {
  ...
  it('throws when no remote configured and displays help', async () => {
    ...
    try {
      await runTasks({
        pkg: { version: '1.0.0', repository: { url: 'http://github.com/user/repo.git' } },
  ...
  it('checks npm user', async () => {
    ...
    try {
      await runTasks({
        pkg: { version: '1.0.0', repository: 'github://test/test' },
        releaseVersion: '2.0.0'
      })
    } catch (error) {
      expect(error.message).toMatch(/Invalid npm user. \"null\" should be \"test\"./)
    }
  })
