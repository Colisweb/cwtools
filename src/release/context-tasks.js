...
import { owner as getOwner } from 'repo-utils'
      ...
      task: async () => {
        try {
          ...
          if (error.message.match(/No remote repository specified/)) {
            ...
            if (upstreamUrl) {
              upstreamUrl = url.format(Object.assign(url.parse(upstreamUrl), { protocol: 'https' }))
              throw new Error(
                `No remote repository configured. Run \`git remote add origin ${upstreamUrl}; git push -u origin master\``
              )
            }
